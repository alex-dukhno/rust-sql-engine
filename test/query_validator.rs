...
fn validate_create_already_existed_table() {
    ...
    drop(evaluate_query("create table table1 (col2 integer);", &DataManager::default(), &catalog_manager));
...
fn validate_insertion_when_column_type_does_not_match() {
    ...
    drop(evaluate_query("create table table_name (col integer);", &DataManager::default(), &catalog_manager));
