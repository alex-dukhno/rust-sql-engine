...
fn creates_single_column_table() {
    ...
    expect!(executer.execute("create table table_name (col int);".tokenize().parse()))
...
fn creates_a_table_with_list_of_columns() {
    let executer = QueryExecuter::default();
    expect!(executer.execute("create table table_name (col1 int, col2 int, col3 int);".tokenize().parse()))
        .to(be_ok().value(ExecutionResult::Message("'table_name' was created".to_owned())));
}
...
fn inserts_row_in_created_table() {
    let executer = QueryExecuter::default();
    drop(executer.execute("create table table_name (col int);".tokenize().parse()));

    expect!(executer.execute("insert into table_name values(1);".tokenize().parse()))
...
fn inserts_row_in_table_with_many_columns() {
    let executer = QueryExecuter::default();
    drop(executer.execute("create table table_name (col1 int, col2 int);".tokenize().parse()));

    expect!(executer.execute("insert into table_name values(1, 2);".tokenize().parse()))
        .to(be_ok().value(ExecutionResult::Message("row was inserted".to_owned())));
}
...
fn does_not_insert_when_column_type_does_not_match() {
    ...
    expect!(executer.execute("insert into table_name values('string');".tokenize().parse()))
...
fn selects_inserted_data_from_table() {
    ...
    drop(executer.execute("create table table_name (col int);".tokenize().parse()));
    drop(executer.execute("insert into table_name values(1);".tokenize().parse()));

    expect!(executer.execute("select col from table_name;".tokenize().parse()))
        .to(be_ok().value(ExecutionResult::Data(vec![vec!["1".to_owned()]])));

    drop(executer.execute("insert into table_name values(2);".tokenize().parse()));
